...
class MainGraph():
    ...
    def _get_programmer_evaluator_agent(self) -> ProgrammerEvaluatorAgent:
        ...
        prog_eva_version = 'v1'
        prog_eva_tools   = [execute_binaries, read_rag_document, read_feedback, store_rag_document]
    ...
    def _programmer_node_action(self, state: AgentState) -> AgentState:
        ...
        # Consume new retrievals exactly once
        if state.get('retrieval_responses'):
            state['conversation'].extend(state['retrieval_responses'])
            state['retrieval_responses'].clear() 
